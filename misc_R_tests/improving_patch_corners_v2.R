  ...
  if(NA_tol>0){
    ...
    if(length(inds)>0){
      for(i in inds){
        ...
        #top left corner isn't working at all now--need to investigate further
        #might just need to multiply angles by -1 in certain cases by the looks of it...
        #but how to catch and fix this???
        debug(.magic.corner.patcher)
        plot(.magic.corner.patcher(terra::geom(terra::union(x[i+1],NA.patch1)[3]),
                                   top=TRUE,left=TRUE,tol=NA_tol)[,c("x","y")],type="l")

        #bottom right corner works for Canada, but not America
        plot(.magic.corner.patcher(terra::geom(terra::union(x[i+1],NA.patch2)[3]),
                                   top=FALSE,left=FALSE,tol=NA_tol)[,c("x","y")],type="l")

        #bottom left corner works
        plot(.magic.corner.patcher(terra::geom(terra::union(x[which(terra::values(x)$SOVEREIGNT=="Argentina")],SA.patch)[3]),
                                   top=FALSE,left=TRUE,tol=SA_tol)[,c("x","y")],type="l")

        #top right corner works
        plot(.magic.corner.patcher(terra::geom(terra::union(x[which(terra::values(x)$SOVEREIGNT=="China")],Asia.patch)[3]),
                                   top=TRUE,left=FALSE,tol=Asia_tol)[,c("x","y")],type="l")
        ...
        tmp<-terra::aggregate(terra::union(x[i],
                                           terra::vect(.magic.corner.patcher(),"polygons")))
        terra::values(tmp)<-terra::values(x)[i,,drop=FALSE]
        x<-rbind(x,tmp)
      }
      x<-x[-inds]
